...
test("live chats reuse the delete strategy with target-specific messages", () => {
  ...
  const target = context.YtActivityCleanerShared.getTargetById("liveChats");
  ...
  assert.equal(strategy.getCompletedCountMessage(2), "Deleted live chat messages: 2");
  assert.equal(
    strategy.getNoMoreActionsMessage(),
    "No more visible live-chat delete buttons were found."
  );
});
