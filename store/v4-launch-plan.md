...
| --- | --- | --- | --- |
| Core cleaner flows | Done | Comments, comment likes, and liked videos are supported with retry handling, counters, and target-aware messages | Run live manual smoke tests on each target before release |
...
| Manual QA | To do | A smoke checklist exists for comments, comment likes, and liked videos in `en` and `pl` | Run the checklist and record the results before packaging |
...
1. Run the `en` and `pl` smoke test checklist across comments, comment likes, and liked videos.
